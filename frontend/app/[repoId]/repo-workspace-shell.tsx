...
/** 同源预览代理，避免 iframe 直接访问 localhost 被 HTTPS / 内嵌浏览器拦截 */
...
export function RepoWorkspaceShell({
  ...
  const [sandboxOverrides, setSandboxOverrides] = useState<
    Record<string, { previewUrl: string; devCommandTerminalUrl: string }>
  >({});
  ...
  // Poll AI service sandbox status for repos that have no vm info from backend
  useEffect(() => {
    ...
    void (async () => {
      const results: Record<
        string,
        { previewUrl: string; devCommandTerminalUrl: string }
      > = {};
      await Promise.all(
        reposWithoutVm.map(async (repo) => {
          try {
            const res = await fetch(
              `/api/sandbox-status/${repo.id}`,
...
function AppPreview({
  ...
  // Always proxy through Next.js /api/sandbox-preview so the browser
  // never loads a raw localhost URL directly (avoids HTTPS/mixed-content).
  // The proxy route injects <base href="/api/sandbox-preview/<repoId>/"> into HTML
  // so relative assets like style.css resolve under the correct repo prefix.
  ...
  }, [metadata!.previewUrl, proxyRoot]);

  const metaAdditional = "additionalTerminalsUrl" in metadata!
...
function BrowserControls({
  ...
  const navigate = (path: string) => {
    ...
    if (repoId) {
      ...
      if (cleanPath) {
        base.pathname = `${base.pathname.replace(/\/$/, "")}/${cleanPath.split("/").map(encodeURIComponent).join("/")}`;
      }
